...
class linear_regression:  # wrapper for the linear regression utilities
    ...
    def fit(self, X, y):
        ...
        self.y = y        
        # (X.T * X).inverse() * X.T * y
        term1 = (np.dot(X.T, X) + self.l * np.identity(len(X.T))).I 
        mat_w = np.dot(np.dot(term1, X.T), y)
        self.w = np.asarray(mat_w).flatten()  # Restablish as a 1 dim ndarray
        return self.w  
        
    def predict(self, X):
        yhat = np.dot(X, self.w.T)
...
if __name__ == "__main__":
    ...
    if len(sys.argv)<2:
        ...
        model.fit(np.asmatrix(train_data), train_labels)  # fit the model
        predictions = model.predict(np.asmatrix(test_data))  # predict
